<a href="https://www.gitanimals.org/">
      <img
        src="https://render.gitanimals.org/guilds/673440973112302533/draw"
        width="600"
        height="300"
        alt="gitanimals"
      />
    </a>
<br>

## Hi, I'm Ryu 

### Open Source Contributions
- [spring-data-jpa](https://github.com/spring-projects/spring-data-jpa/pull/4195) - Flatten nested AND/OR predicates in the JPQL query builder (with regression tests).
- [spring-data-jpa](https://github.com/spring-projects/spring-data-jpa/pull/4193) - Fix typos in test Javadoc.
- [spring-data-redis](https://github.com/spring-projects/spring-data-redis/pull/3317) - Optimize spin-wait retry loop in RedisMessageListenerContainer
- [spring-boot](https://github.com/spring-projects/spring-boot/pull/49278) - Remove obsolete idle timeout configuration in Jetty tests and simplify test setup.

## ☁️ SNS 

[![Tistory](https://img.shields.io/badge/Tistory-000000?style=for-the-badge&logo=Blogger&logoColor=white)](https://toy-garden.tistory.com/)

----
[![Solved.ac프로필](http://mazassumnida.wtf/api/v2/generate_badge?boj=ehjwys)](https://solved.ac/ehjwys)
